...
//    key => Option(modules.config).flatMap(conf => Option(conf.getString(key)))
//  )
//
//  val getConfigMillis = wrap[Long](
//    key => Option(modules.config).flatMap(conf => Option(conf.getLong(key)))
//  )
//
//  val HEADER_NAME: String = getConfigString("play.http.session.jwtName").getOrElse("Authorization")
//
//  val MAX_AGE: Option[Long] = getConfigMillis("play.http.session.maxAge")
//
