...
class ShareRoute(modules: ConfigurationModule with PersistenceModule with BusinessModule with RoutesModuleImpl) extends Directives with SqlProcessor {
  ...
  @ApiImplicitParams(Array(
    ...
    new ApiImplicitParam(name = "expiration", value = "days", required = true, dataType = "integer", paramType = "query")
  ...
  def getShareURLRoute: Route = path(routeName / LongNumber) { widgetId =>
    get {
      parameter('expiration.as[Long]) { expiration =>

        authenticateOAuth2Async[SessionClass](AuthorizationProvider.realm, AuthorizationProvider.authorize) {
          session => getShareURLComplete(session, widgetId, expiration)
        }
      }
    }
  }
  ...
  private def getShareURLComplete(session: SessionClass, widgetId: Long, expiration: Long) = {
    onComplete(shareService.getWidgetById(widgetId)) {
      case Success(widgetInfo) =>
        ...
        val flatTableId = putWidgetInfo.flatTable_id
        val widgetJsonStr = caseClass2json(PutWidgetInfo(widgetInfo._1, widgetInfo._2, widgetInfo._3, widgetInfo._4, widgetInfo._5.getOrElse(""), widgetInfo._6, widgetInfo._7.getOrElse(""), widgetInfo._8, Some(widgetInfo._9)))
            ...
            if (sourceInfo.nonEmpty) {
              try {
                ...
                val sqls = sqlParam.map(param => {
                  getSqlArr(sqlTemp, param, tableName, putWidgetInfo.adhoc_sql)
                }).mkString("")
                val identifier = java.util.UUID.randomUUID().toString
                val shareInfo = ShareInfo(0, identifier, sqls, widgetJsonStr, connectionUrl, currentTime, session.userId, expiration)
                onComplete(modules.shareInfoDal.insert(shareInfo)) {
                  case Success(_) => complete(OK, ResponseJson[Identifier](getHeader(200, session), Identifier(identifier)))
